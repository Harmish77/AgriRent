...
if (!isset($_SESSION['logged_in']) || $_SESSION['logged_in'] !== true || $_SESSION['user_type'] !== 'F') {
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['place_order'])) {
    ...
    error_log("Order form submitted for product ID: " . $product_id);
    ...
    // Calculate total pending orders for this product
    if (empty($errors)) {
        $pending_query = "SELECT SUM(quantity) as total_pending 
                         FROM product_orders 
                         WHERE Product_id = ? AND Status = 'PEN'";
    ...
    if (empty($errors)) {
        $dup_check = $conn->prepare("SELECT Order_id FROM product_orders WHERE Product_id = ? AND buyer_id = ? AND Status = 'PEN'");
        $dup_check->bind_param('ii', $product_id, $customer_id);
        $dup_check->execute();
        if ($dup_check->get_result()->num_rows > 0) {
            $errors[] = 'You already have a pending order for this product. Please wait for confirmation or cancel the existing order.';
        }
        $dup_check->close();
    }
    ...
    if (empty($errors)) {
        ...
        $insert_sql = "INSERT INTO product_orders (Product_id, buyer_id, quantity, total_price, Status, order_date) 
                       VALUES (?, ?, ?, ?, 'PEN', NOW())";
        
        $stmt = $conn->prepare($insert_sql);
        $stmt->bind_param('iidd', $product_id, $customer_id, $quantity, $total_price);
            ...
            <?php if ($success_message): ?>
                <div style="background: #d4edda; color: #155724; padding: 20px; border-radius: 8px; margin-bottom: 30px; border: 1px solid #c3e6cb;">
                    ...
                    <div style="margin-top: 15px;">
                        <a href="farmer/orders.php" class="btn" style="background: #28a745; color: white; padding: 10px 20px; text-decoration: none; border-radius: 4px; margin-right: 10px;">View My Orders</a>
