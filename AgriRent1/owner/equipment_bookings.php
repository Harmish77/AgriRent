...
// Fetch bookings for this owner's equipment with time_slot
$bookings = [];
try {
    $query = "SELECT eb.booking_id, eb.equipment_id, eb.customer_id, eb.start_date, eb.end_date, 
                     eb.Hours, eb.total_amount, eb.status, eb.time_slot, e.Title as equipment_title, 
    ...
    <?php if (count($bookings) > 0): ?>
        ...
        <?php if ($booking['status'] == 'PEN'): ?>
                                <div style="display: flex; flex-direction: column; gap: 5px;">
                                    <a href="?action=approve&id=<?= $booking['booking_id'] ?>&status=<?= $status_filter ?>" 
                                       ...
                                       style="color: #28a745; font-weight: 600; text-decoration: none;">
                                        Approve
                                    </a>
        ...
        <?php else: ?>
                                <a href="booking_details.php?id=<?= $booking['booking_id'] ?>" 
                                   style="color: #17a2b8; font-weight: 600; text-decoration: none;">
                                   View Details
                                </a>
        <?php endif; ?>
